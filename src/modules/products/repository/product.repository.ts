...
export class ProductsRepository {
  ...
  async create(createProductDto: CreateProductDto): Promise<InterfaceProduct> {
    ...
    const result: QueryResult<InterfaceProduct> = await this.pool.query(SQL.INSERT_PRODUCT, [
      name,
      price,
    ]);
  ...
  async findOne(id: string): Promise<InterfaceProduct> {
    const result: QueryResult<InterfaceProduct> = await this.pool.query(SQL.SELECT_ONE_PRODUCT, [
      id,
    ]);
    return result.rows[0];
  }
